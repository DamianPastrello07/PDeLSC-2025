...
const Navbar = ({ user, setUser }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await api.post('/auth/login', { email, password }); // ajusta la ruta si es diferente
      localStorage.setItem('token', res.data.token);
      setUser({ token: res.data.token, email: res.data.email }); // si el backend devuelve email
      setEmail('');
      setPassword('');
    } catch (err) {
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          ...
          {!user ? (
            ...
            <div className="d-flex align-items-center">
              <span className="text-light me-3">{user.email || 'Usuario'}</span>
